...
SUPABASE_KEY = os.getenv(
    "SUPABASE_KEY",
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InR5dW5xdGhvaW5hbWRseWhnbXVxIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc4MDk2NjM5NSwiZXhwIjoyMDk2NTQyMzk1fQ.bVjJQbQnbVHp9rNAR7Ju7Iu6ioB5Gj-S_Q5Jw-5V9Mw",
)
...
if SUPABASE_KEY == "TU_SERVICE_ROLE_KEY_ACTUALIZADA" or not SUPABASE_KEY:
    ...
    print(
        "   asegúrate de configurar correctamente tu archivo .env o la variable de entorno."
    )
...
def parsear_norma_real(pdf_path: str):
    ...
    try:
        ...
        print(f"❌ Error crítico leyendo el archivo PDF: {e!s}")
        return []
    ...
    for num_pag in range(rango_maximo):
    ...
    for cod_control, info in controles_encontrados.items():
        ...
        quiz_dinamico = {
            ...
            "options": [
                {
                    "id": "A",
                    "text": "Aplicar de forma estricta las salvaguardas y registrar los logs de auditoría sin omisiones.",
                },
                {
                    "id": "B",
                    "text": "Deshabilitar las restricciones perimetrales para acelerar las respuestas de la API.",
                },
                {
                    "id": "C",
                    "text": "Almacenar tokens de autenticación en texto plano dentro de la base de datos de pruebas.",
                },
            ],
            "correct": "A",
        }
...
def subir_a_supabase(datos):
    if not datos:
        print(
            "⚠️ No se generaron módulos para inyectar. Verifica el archivo PDF o las páginas analizadas."
        )
        return
    ...
    for idx, item in enumerate(datos):
        ...
        try:
            ...
            print(
                f"✅ [{item['domain_id']}] {item['title']} sincronizado [UUID: {lesson_id[:8]}...]"
            )
            exitos += 1

        except Exception as e:
            print(f"❌ Error inyectando el control '{item['title']}': {e!s}")
            errores += 1
...
if __name__ == "__main__":
    ...
    if os.path.exists(pdf_norma):
        ...
        print(
            "💡 Sugerencia: Asegúrate de colocar el archivo PDF 'NIST.SP.800-53r5.pdf' en la misma carpeta que este script."
        )
