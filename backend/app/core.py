...
async def get_current_user(
    ...
    try:
        ...
        raise credentials_exception
    user = db.execute(
        text("SELECT email, role FROM usuarios WHERE email = :email"),
...
def require_roles(allowed_roles: list[str]):
    async def role_checker(current_user: dict = Depends(get_current_user)):
        if current_user["role"] not in allowed_roles:
            raise HTTPException(
                status_code=403, detail="No tienes permisos para esta operación"
            )
        return current_user
...
class PasswordRecovery2FARequest(BaseModel):
    ...
    token: str = Field(
        ..., description="Código TOTP de 6 dígitos para validar identidad"
    )
...
async def registrar_log(
    ...
    try:
        ...
        db.rollback()
        print(f"🚨 CRITICAL: Falló el registro del log de auditoría: {e!s}")
