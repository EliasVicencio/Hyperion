...
async def login(
    ...
    try:
        query = text(
            "SELECT password, role, two_factor_enabled FROM usuarios WHERE email = :email"
        )
        ...
        if not user_record or not verify_password(password, user_record[0]):
            await registrar_log(
                ...
                "LOGIN_FAILED",
                "WARN",
                detalles="Intento fallido de autenticación.",
            )
            raise HTTPException(
                status_code=400,
        ...
        await registrar_log(
            ...
            "LOGIN_SUCCESS",
            "INFO",
            detalles="Inicio de sesión perimetral correcto.",
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error en el proceso de autenticación: {e!s}"
        )
...
async def verify_2fa(
    request: Request, data: TokenVerifyRequest, db: Session = Depends(get_db)
):
    import pyotp

    try:
        query = text(
            "SELECT two_factor_secret, role FROM usuarios WHERE email = :email"
        )
        user_record = db.execute(query, {"email": data.username}).fetchone()
        if not user_record or not user_record[0]:
            raise HTTPException(
                status_code=400,
                detail="Este operador no tiene configurada una clave TOTP.",
            )
        ...
        if not totp.verify(data.token):
            await registrar_log(
                ...
                "2FA_FAILED",
                "WARN",
                detalles="Fallo de código de verificación 2FA.",
            )
            raise HTTPException(
                status_code=400, detail="Código de seguridad inválido o expirado."
            )
        role = (user_record[1].upper() + "_ROLE") if user_record[1] else "OPERADOR_ROLE"
        access_token = create_access_token(data={"sub": data.username, "role": role})
        await registrar_log(
            db, data.username, "2FA_SUCCESS", "INFO", detalles="Doble factor validado."
        )
        return {
            "status": "success",
            "access_token": access_token,
        ...
        raise HTTPException(
            status_code=500, detail=f"Fallo en verificación perimetral: {e!s}"
        )
...
async def update_password(
    payload: PasswordUpdateRequest, db: Session = Depends(get_db)
):
    if not RAW_DB_URL:
        raise HTTPException(
            status_code=500, detail="Error de configuración: DATABASE_URL vacía."
        )
    try:
        check_user_query = text("SELECT id FROM usuarios WHERE email = :email")
        user_exists = db.execute(
            check_user_query, {"email": payload.username}
        ).fetchone()
        if not user_exists:
            raise HTTPException(
                status_code=404,
                detail="El operador especificado no reside en el sistema.",
            )
        ...
        await run_in_threadpool(
            db.execute,
            update_query,
            {"password": new_hashed_password, "email": payload.username},
        )
        db.commit()
        await registrar_log(
            db,
...
async def recover_password_via_2fa(
    payload: PasswordRecovery2FARequest, db: Session = Depends(get_db)
):
    import pyotp
    ...
    try:
        query = text(
            "SELECT two_factor_secret, two_factor_enabled FROM usuarios WHERE email = :email"
        )
        user_record = db.execute(query, {"email": payload.username}).fetchone()
        if not user_record:
            raise HTTPException(
                status_code=400,
                detail="Verificación de identidad fallida o parámetros inválidos.",
            )
        ...
        if not totp.verify(payload.token):
            await registrar_log(
                ...
                "RECOVERY_FAILED",
                "WARN",
                detalles="Código TOTP de recuperación inválido.",
            )
            raise HTTPException(
                status_code=400, detail="Código de seguridad inválido o expirado."
            )
        ...
        return {
            "status": "success",
            "message": "Credenciales actualizadas correctamente en Hyperion Core.",
        }
    except HTTPException:
        raise
    except Exception as e:
        db.rollback()
        raise HTTPException(
            status_code=500, detail=f"Error en pasarela de recuperación: {e!s}"
        )
...
async def setup_2fa(username: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        totp_auth_url = pyotp.totp.TOTP(secret).provisioning_uri(
            name=username, issuer_name="Hyperion Core"
        )
        query = text(
            "UPDATE usuarios SET two_factor_secret = :secret, two_factor_enabled = FALSE WHERE email = :email"
        )
        ...
        db.rollback()
        raise HTTPException(
            status_code=500, detail=f"Error al generar semilla TOTP: {e!s}"
        )
...
async def activate_2fa(data: TokenVerifyRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if not user_record or not user_record[0]:
            raise HTTPException(
                status_code=400, detail="Semilla TOTP no inicializada para esta cuenta."
            )
        totp = pyotp.TOTP(user_record[0])
        if not totp.verify(data.token):
            raise HTTPException(
                status_code=400,
                detail="El código de confirmación no coincide con el autenticador.",
            )
        query_update = text(
            "UPDATE usuarios SET two_factor_enabled = TRUE WHERE email = :email"
        )
        ...
        await registrar_log(
            ...
            "2FA_ACTIVATED",
            "INFO",
            detalles="El operador activó el resguardo por token TOTP.",
        )
        return {
            "status": "activated",
            "message": "Autenticación de Dos Factores vinculada correctamente al sistema.",
        }
    except HTTPException:
        raise
    except Exception as e:
        db.rollback()
        raise HTTPException(
            status_code=500, detail=f"Error en activación de seguridad: {e!s}"
        )
...
async def deactivate_2fa(data: TokenVerifyRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if not user_record or not user_record[0]:
            raise HTTPException(
                status_code=400,
                detail="No hay una configuración 2FA activa en este usuario.",
            )
        totp = pyotp.TOTP(user_record[0])
        if not totp.verify(data.token):
            raise HTTPException(
                status_code=400, detail="Código de desactivación incorrecto."
            )
        query_update = text(
            "UPDATE usuarios SET two_factor_enabled = FALSE, two_factor_secret = NULL WHERE email = :email"
        )
        db.execute(query_update, {"email": data.username})
        db.commit()
        await registrar_log(
            ...
            "2FA_DEACTIVATED",
            "CRITICAL",
            detalles="Doble factor removido por el operador.",
        )
        return {
            "status": "deactivated",
            "message": "Autenticación de dos factores removida.",
        }
    except HTTPException:
        raise
    except Exception as e:
