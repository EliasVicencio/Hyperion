...
class NuevoTicketExterno(NuevoTicket):
    solicitante: str = Field(
        ...,
        description="Nombre o email de quien reporta, ya que no hay sesión de Hyperion",
    )
...
def _validar_prioridad(prioridad: str) -> str:
    ...
    if p not in PRIORIDADES_VALIDAS:
        raise HTTPException(
            status_code=422,
            detail=f"Prioridad inválida. Usa una de: {', '.join(PRIORIDADES_VALIDAS)}",
        )
    return p


async def _crear_ticket_en_bd(
    db: Session,
    titulo: str,
    descripcion: str,
    prioridad: str,
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(
            status_code=500, detail=f"Excepción en la base de datos: {e!s}"
        )
    ...
    await registrar_log(
        ...
        "TICKET_CREATED",
        "INFO",
        detalles=f"Ticket #{ticket['id']} ({prioridad}): {titulo}",
    )
...
async def crear_ticket(
    ...
    return await _crear_ticket_en_bd(
        db,
        payload.titulo,
        payload.descripcion,
        prioridad,
...
async def crear_ticket_externo(
    ...
    if not TICKETS_API_KEY:
        raise HTTPException(
            status_code=500, detail="TICKETS_API_KEY no está configurada en el backend."
        )
    if not x_api_key or x_api_key != TICKETS_API_KEY:
        raise HTTPException(
            status_code=401, detail="API key inválida o ausente (header X-API-Key)."
        )
...
async def actualizar_ticket(
    ...
    if not row:
        raise HTTPException(
            status_code=404, detail=f"No existe el ticket #{ticket_id}."
        )
    ...
    await registrar_log(
        ...
        "TICKET_UPDATED",
        "INFO",
        detalles=f"Ticket #{ticket_id} → {estado}",
    )
