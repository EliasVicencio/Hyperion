...
from ..core import RAW_DB_URL, ProgresoLeccionPayload, get_db, registrar_log
...
async def obtener_plan_estudio_nist():
    try:
            ...
            "modulos": [
                {
                    ...
                    "lecciones": [
                        {
                            "id": "au-1",
                            "titulo": "1. Introducción a la directiva AU-2 (Eventos Auditables)",
                            "duracion": "6 min",
                            "completada": True,
                            "contenido": "La directiva AU-2 establece qué acciones del sistema DEBEN registrarse obligatoriamente. En Hyperion Core, esto incluye inicios de sesión, cambios de privilegios, volcados de bases de datos y bloqueos del firewall perimetral. Cada evento debe capturar de manera unívoca: qué ocurrió, cuándo ocurrió (timestamp), dónde ocurrió (nodo de origen) y quién lo provocó (actor).",
                        },
                        {
                            "id": "au-2",
                            "titulo": "2. Monitoreo y Trawzabilidad bajo el control AU-6",
                            "duracion": "8 min",
                            "completada": True,
                            "contenido": "El control AU-6 exige una revisión y correlación continua de los registros de auditoría para detectar comportamientos inusuales o ataques. No basta con almacenar los logs; el sistema debe contar con analíticas automáticas que correlacionen eventos aislados (por ejemplo, múltiples llamadas de API fallidas seguidas de una exportación de BD) para emitir alertas de mitigación en tiempo real.",
                        },
                        {
                            "id": "au-3",
                            "titulo": "3. Criptografía y Blockchain: Profundizando en AU-9",
                            "duracion": "12 min",
                            "completada": False,
                            "contenido": "El control AU-9 (Integridad de Registros) es el núcleo criptográfico de Hyperion. Exige que los registros estén protegidos contra modificaciones no autorizadas. Implementamos esto mediante un encadenamiento de bloques SHA-256 (lógica blockchain): cada log almacena el hash del bloque anterior. Si un atacante altera una fila directamente en PostgreSQL, la firma digital del bloque se rompe, invalidando la cadena completa inmediatamente.",
                        },
                    ],
                },
                {
                    ...
                    "lecciones": [
                        {
                            "id": "ac-1",
                            "titulo": "1. Control AC-2: Gestión de Cuentas de Privilegio",
                            "duracion": "7 min",
                            "completada": False,
                            "contenido": "Regula el ciclo de vida de las cuentas del sistema. Las cuentas administrativas (como sysadmin) deben auditarse rigurosamente bajo el principio de 'menor privilegio posible'. Ningún operador debe poseer permisos permanentes para modificar la estructura de gobernanza sin una ventana de tiempo aprobada.",
                        },
                        {
                            "id": "ia-2",
                            "titulo": "2. Mecanismos de Autenticación Multifactor (MFA/TOTP)",
                            "duracion": "10 min",
                            "completada": False,
                            "contenido": "El control IA-2 dictamina que todo acceso remoto o local a sistemas federales críticos requiere autenticación de factores independientes. Hyperion integra algoritmos TOTP (Time-Based One-Time Password) mediante tokens criptográficos de 6 dígitos que expiran cada 30 segundos, neutralizando ataques de reutilización de credenciales.",
                        },
                    ],
                },
                {
                    ...
                    "lecciones": [
                        {
                            "id": "si-1",
                            "titulo": "1. Control SI-4: Monitoreo de Alertas Perimetrales",
                            "duracion": "9 min",
                            "completada": False,
                            "contenido": "Establece los requisitos para el análisis del tráfico de red entrante y saliente. El sistema busca firmas conocidas de ataques e indicadores de compromiso (IoC). Cuando nuestro firewall mitiga una inyección SQL en la API, actúa bajo el amparo estricto de este control federal.",
                        }
                    ],
                },
            ],
        ...
        raise HTTPException(
            status_code=500, detail=f"Internal database mismatch: {e!s}"
        )
...
async def registrar_progreso_leccion(payload: ProgresoLeccionPayload):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Database error writing ledger: {e!s}"
        )
...
@router.get("/cursos")
async def listar_cursos_academia(
    operador_email: str = "operador-root", db: Session = Depends(get_db)
):
    if not RAW_DB_URL:
        return {
            "iso_27001": [
                {
                    "id": 1,
                    "titulo": "Estructura del SGSI y Ciclo PHVA",
            ...
            "nist_80053": [
                {
                    "id": 3,
                    "titulo": "Hardening e Integridad de Registros (Familia AU)",
    ...
    try:
        db.execute(
            text("SET LOCAL app.current_operator_email = :email"),
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Fallo en la pasarela de extracción de la Academia: {e!s}",
        )


@router.post("/progreso")
async def actualizar_progreso_academia(
    payload: ProgresoLeccionPayload,
    operador_email: str = "operador-root",
    ...
    try:
        query_curso = text(
            "SELECT id, total_lecciones FROM academia_cursos WHERE id = :id"
        )
        curso = db.execute(query_curso, {"id": int(payload.modulo_id)}).fetchone()
        if not curso:
            raise HTTPException(
                status_code=404, detail="El curso regulatorio especificado no existe."
            )
        ...
        result = db.execute(
            query_upsert,
            {"curso_id": curso_id, "email": operador_email, "total": total_lecciones},
        )
