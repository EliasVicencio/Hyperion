...
async def verificar_cadena_criptografica(
    db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)
):
    try:
        query = text(
            "SELECT id, operador, accion, categoria, detalles, timestamp FROM logs_auditoria ORDER BY id ASC"
        )
        ...
        for row in result:
            ...
            payload_combinado = (
                f"{id_log}-{operador}-{accion}-{categoria}-{detalles_str}-{hash_previo}"
            )
            hash_calculado = hashlib.sha256(
                payload_combinado.encode("utf-8")
            ).hexdigest()
            logs_procesados.append(
                {
                    "id": id_log,
                    "control": "A.8.15 / A.8.24"
                    if categoria in ["CRITICAL", "WARN"]
                    else "A.8.15",
                    "event_type": accion,
                    "actor": operador,
...
async def simular_ataque_bd(
    ...
    try:
        ...
        query = text(
            "UPDATE logs_auditoria SET detalles = '🚨 ATAQUE: Registro mutado mediante inyección perimetral SQL.' WHERE id = :id"
        )
...
async def restaurar_cadena_criptografica(
    db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)
):
    try:
        query_ataque = text("""
        ...
        for row in logs_ataque:
            ...
            detalles_limpios = (
                "Doble factor validado de forma exitosa mediante canal seguro."
            )
            update_query = text("""
        ...
        return {
            "status": "RESTORED",
            "message": "Cadena corregida y mitigada con éxito.",
        }
    except Exception as e:
        db.rollback()
        raise HTTPException(status_code=500, detail=str(e))
