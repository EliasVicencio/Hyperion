...
async def _crear_operador_en_bd(payload: NuevoOperador, db: Session):
    if not RAW_DB_URL:
        raise HTTPException(
            status_code=500,
            detail="Error de configuración: La variable DATABASE_URL está vacía en Vercel.",
        )
    ...
    try:
        ...
        await registrar_log(
            ...
            "OPERADOR_CREATED",
            "WARN",
            detalles=f"Alta de nueva identidad por el sistema. Rol: {payload.role}",
        )
        return {
            "id": row[0],
            ...
            "activo": True,
            "ultima_conexion": row[4].strftime("%Y-%m-%d %H:%M:%S")
            if row[4]
        ...
        db.rollback()
        raise HTTPException(
            status_code=409, detail="Ya existe un operador registrado con ese email."
        )
...
async def crear_operador(
    payload: NuevoOperador,
    db: Session = Depends(get_db),
    admin: dict = Depends(require_roles(["admin", "ADMIN_ROLE"])),
):
    return await _crear_operador_en_bd(payload, db)
...
async def eliminar_operador(
    ...
    try:
        ...
        await registrar_log(
            ...
            "ACCESS_REVOKED",
            "CRITICAL",
            detalles=f"Purga de credenciales completada para el ID {id}.",
        )
        return {
            "status": "success",
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error en la base de datos al purgar registro: {e!s}",
        )
