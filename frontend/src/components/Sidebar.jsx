...
export default function Sidebar({ currentPage, setPage, onLogout }) { // 👈 Recibe onLogout aquí
  const menu = [
  ...
  return (
    <div className="w-64 h-screen bg-[#050810] border-r border-slate-900/50 flex flex-col fixed left-0 top-0 z-50">
        ...
        <nav className="space-y-1">
          {menu.map((item) => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all group ${currentPage === item.id
      ...
      <div className="mt-auto p-6 border-t border-slate-900/50">
        <button
          onClick={onLogout} // 👈 Se vincula la acción de cierre de sesión
          className="flex items-center gap-3 text-slate-500 hover:text-red-400 text-sm font-medium w-full transition-colors"
        >
          <LogOut size={18} />
          Cerrar Sistema
        </button>
