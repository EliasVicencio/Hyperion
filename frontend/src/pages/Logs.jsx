...
import { FileSpreadsheet, Search, Terminal, AlertTriangle, Info, ShieldAlert, Download } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <header className="flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-500 text-sm">Registro inmutable de eventos del sistema e historial de acciones administrativas</p>
        </div>
        <button className="bg-slate-900 hover:bg-slate-800 text-slate-300 border border-slate-800 px-4 py-2 rounded-xl text-sm font-semibold flex items-center gap-2 transition-all">
          <Download size={16} /> Exportar CSV
        </button>
      </header>

      {/* Barra de Herramientas: Filtros + Buscador */}
      <div className="flex flex-col md:flex-row gap-4 justify-between items-center bg-[#0b111e] border border-slate-800/50 p-4 rounded-2xl shadow-xl">
        ...
        <div className="relative w-full md:w-72">
          ...
          <input
            ...
            className="bg-slate-950 border border-slate-900 rounded-xl py-2 pl-9 pr-4 w-full text-xs text-slate-300 focus:border-purple-500 outline-none transition-all"
            value={busqueda}
                ...
                <tr key={log.id} className="hover:bg-purple-500/[0.01] transition-colors font-mono">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-col font-sans">
                      <span className="text-slate-200 font-medium">{log.usuario}</span>
                      <span className="text-[10px] font-mono text-slate-500">{log.ip}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-slate-300 font-sans">{log.accion}</td>
                  <td className="px-6 py-4 text-slate-500 text-xs">{log.timestamp}</td>
                </tr>
              ))
            ) : (
              <tr>
                <td colSpan="5" className="px-6 py-12 text-center text-slate-500 italic font-sans">
