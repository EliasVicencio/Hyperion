...
export default function Gobernanza() {
  ...
  // Estados para el Test de Integridad Masivo (NIST AU-9)
  ...
  const simularInyeccionMaliciosa = async () => {
    try {
      ...
      setAuditStatus("COMPROMISED");
      alert(`🚨 Simulación ejecutada: Registro ID #${data.target_id} manipulado directamente en la base de datos. Ejecuta el test NIST para comprobar la ruptura de hashes.`);
  ...
  return (
    // 🌟 CAMBIO: Adaptado texto base adaptativo para modo claro/oscuro
    <div className="space-y-6 text-slate-800 dark:text-slate-300">
      
      {/* Encabezado Principal */}
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div>
          {/* 🌟 CAMBIO: De text-white a text-slate-900 dark:text-white */}
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Garantía de inmutabilidad de registros y correlación criptográfica bajo el estándar <b>NIST SP 800-53 Rev. 5</b></p>
        </div>
        
        <div className="flex gap-2">
          {/* 🌟 CAMBIO: Botón de ataque simulado adaptativo con mejor contraste */}
          <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        {/* 🌟 CAMBIO: Fondo bg-white en claro y bordes suaves adaptativos */}
        <div className={`p-4 rounded-2xl border bg-white dark:bg-slate-900/40 backdrop-blur-sm flex items-center gap-4 ${
          ...
          <div>
            <span className="text-[10px] uppercase font-bold tracking-wider text-slate-400 dark:text-slate-500 block">NIST AU-9: Log Integrity</span>
        ...
        {/* 🌟 CAMBIO: Fondo bg-white y texto oscuro text-slate-700 en claro */}
        <div className="p-4 rounded-2xl border border-slate-200 dark:border-slate-800/80 bg-white dark:bg-slate-900/40 backdrop-blur-sm flex items-center gap-4">
          ...
          <div>
            ...
            <span className="text-sm font-mono font-bold text-slate-700 dark:text-slate-200">AU-2, AU-6, AU-9, AU-12</span>
          </div>
        </div>

        {/* Card 3: Custodia criptográfica */}
        {/* 🌟 CAMBIO: Ajuste de textos y fondos claros para sincronía con el dashboard */}
        <div className="p-4 rounded-2xl border border-slate-200 dark:border-slate-800/80 bg-white dark:bg-slate-900/40 backdrop-blur-sm flex items-center gap-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800/80 rounded-2xl p-6 shadow-sm dark:shadow-xl backdrop-blur-sm space-y-4">
          ...
          <div className="relative flex items-center">
            ...
            {/* 🌟 CAMBIO: Input de búsqueda adaptativo */}
            <input
              type="text"
          ...
          <div className="space-y-3 max-h-[460px] overflow-y-auto pr-1">
            {logsFiltrados.length === 0 ? (
              ...
              logsFiltrados.map(log => {
                ...
                // 🌟 CAMBIO: Paleta de colores de badges optimizada para fondos claros y oscuros
                ...
                return (
                  /* 🌟 CAMBIO: Filas adaptativas (bg-slate-50 en modo claro, bg-slate-950 en oscuro al no estar seleccionados) */
                  <div
                    key={log.id}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          {/* 🌟 CAMBIO: text-slate-800 en claro */}
                          <p className="text-slate-800 dark:text-slate-200 font-bold">{log.event_type || "LOG"}</p>
                          <span className="text-[9px] px-1.5 py-0.2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-400 dark:text-slate-500 rounded font-bold font-sans">{log.control || "AU-2"}</span>
                        </div>
        ...
        <div className="space-y-4">
          {selectedLog ? (
            /* 🌟 CAMBIO: Contenedor del inspector adaptativo */
            <div className="bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800/80 rounded-2xl p-5 shadow-sm dark:shadow-xl backdrop-blur-sm space-y-4">
              ...
              <div className="space-y-3 font-mono text-[11px]">
                {/* 🌟 CAMBIO: Cajas internas de hashes adaptadas */}
                <div className="bg-slate-50 dark:bg-slate-950/80 p-3 rounded-xl border border-slate-100 dark:border-slate-900">
                  <span className="text-slate-400 dark:text-slate-500 block text-[9px] font-bold">PREVIOUS_BLOCK_HASH (AU-9):</span>
          ...
          {/* Tarjeta explicativa NIST */}
          {/* 🌟 CAMBIO: Fondo #040712 condicionado solo a oscuro, fondo blanco/slate-50 elegante en claro */}
          <div className="bg-slate-50 dark:bg-[#040712] border border-slate-200 dark:border-slate-900/80 rounded-2xl p-5 space-y-3">
            ...
            <p className="text-[10px] font-mono text-slate-500 leading-relaxed">
              El sistema genera firmas hash encadenadas utilizando <span className="text-slate-700 dark:text-slate-400">PostgreSQL + SHA-256</span>. Cualquier mutación no autorizada romperá la referencia secuencial, revocando el certificado de confianza de inmediato.
            </p>
          </div>
        </div>
