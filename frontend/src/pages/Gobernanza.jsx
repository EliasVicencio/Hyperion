...
export default function Gobernanza() {
  ...
  const cargarVerificacionCadena = async (mostrarAnimacion = false) => {
    ...
    try {
      ...
      if (mostrarAnimacion) {
        // Simulación visual del progreso para mantener el feeling de escaneo perimetral
        let progreso = 0;
        const interval = setInterval(() => {
          ...
          if (progreso >= 100) {
            ...
            // Verificamos si existe alguna mutación maliciosa en los registros
  ...
  // Carga inicial al montar la pestaña para evitar la pantalla negra
  useEffect(() => {
    cargarVerificacionCadena(false);
  }, []);

  // Filtrar logs evitando errores si los campos de la BD vienen nulos
  ...
  // --- EJECUTAR AUDITORÍA CRIPTOGRÁFICA REAL (NIST AU-9) ---
  const ejecutarAuditoriaNIST = () => {
    cargarVerificacionCadena(true);
  };

  // --- SIMULAR INYECCIÓN DIRECTA EN SUPABASE ---
  const simularInyeccionMaliciosa = async () => {
    try {
      ...
      // Refrescamos datos inmediatamente de fondo
      cargarVerificacionCadena(false);
    } catch (error) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="p-4 rounded-2xl border border-slate-800/80 bg-slate-900/40 backdrop-blur-sm flex items-center gap-4">
          ...
          <div>
            ...
            <span className="text-xs font-mono text-slate-400 truncate w-40 block">
        ...
        <div className="lg:col-span-2 bg-slate-900/40 border border-slate-800/80 rounded-2xl p-6 shadow-xl backdrop-blur-sm space-y-4">
          ...
          <div className="space-y-3 max-h-[460px] overflow-y-auto pr-1">
            {logsFiltrados.length === 0 ? (
              <div className="p-8 text-center text-xs font-mono text-slate-600">SISTEMA_SIN_REGISTROS_O_ESPERANDO_API</div>
            ) : (
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <p className="text-slate-200 font-bold">{log.event_type || "LOG"}</p>
                          <span className="text-[9px] px-1.5 py-0.2 bg-slate-900 border border-slate-800 text-slate-500 rounded font-bold font-sans">{log.control || "AU-2"}</span>
                        </div>
            ...
            <div className="bg-slate-900/40 border border-slate-800/80 rounded-2xl p-5 shadow-xl backdrop-blur-sm space-y-4">
              ...
              <div className="space-y-3 font-mono text-[11px]">
                <div className="bg-slate-950/80 p-3 rounded-xl border border-slate-900">
                  ...
                  <span className="break-all text-slate-400 tracking-tighter">{selectedLog.previous_hash || "000000000000..."}</span>
                </div>
                
                <div className="bg-slate-950/80 p-3 rounded-xl border border-slate-900">
                  <span className="text-indigo-400 block text-[9px] font-bold">CURRENT_BLOCK_HASH (SHA-256):</span>
                  <span className="break-all font-bold text-indigo-400 tracking-tighter">{selectedLog.current_hash || "Generando..."}</span>
                </div>
                ...
                <div className="bg-slate-950/40 p-3 rounded-xl border border-slate-900/60 text-slate-300 space-y-2">
                  ...
                  <p className="text-[10px] leading-relaxed"><span className="text-slate-500">Payload:</span> {selectedLog.detalles || "Sin detalles"}</p>
                </div>
              </div>
