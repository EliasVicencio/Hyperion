...
// Datos simulados del estado de los dominios ISO 27001:2022
const datosDominios = [
    ...
    <div className="space-y-6">
      ...
      <header className="flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-500 text-sm">Alineación con el SGSI e inspección de controles del Anexo A de la norma ISO/IEC 27001:2022</p>
        </div>
        <div className="bg-blue-500/10 text-blue-400 border border-blue-500/20 px-4 py-2 rounded-xl text-xs font-mono font-bold shadow-[0_0_15px_rgba(59,130,246,0.05)]">
          Índice Global: <span className="text-white text-sm font-sans pl-1">79.7%</span>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-[#0b111e] border border-slate-800/50 p-6 rounded-3xl shadow-2xl flex flex-col justify-between">
          <div>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center bg-slate-950/40 p-3 rounded-xl border border-slate-900">
                <span className="text-xs text-slate-400 flex items-center gap-2"><CheckCircle2 size={14} className="text-emerald-400" /> Controles Implementados</span>
                <span className="text-xs font-mono font-bold text-slate-200">68 / 93</span>
              </div>
              <div className="flex justify-between items-center bg-slate-950/40 p-3 rounded-xl border border-slate-900">
                <span className="text-xs text-slate-400 flex items-center gap-2"><Clock size={14} className="text-amber-400" /> Controles en Desarrollo</span>
                <span className="text-xs font-mono font-bold text-slate-200">18 / 93</span>
              </div>
              <div className="flex justify-between items-center bg-slate-950/40 p-3 rounded-xl border border-slate-900">
                <span className="text-xs text-slate-400 flex items-center gap-2"><AlertCircle size={14} className="text-red-400" /> Brechas Críticas</span>
                <span className="text-xs font-mono font-bold text-red-400">7</span>
              </div>
            </div>
          </div>
          <p className="text-[10px] text-slate-500 leading-relaxed pt-4 border-t border-slate-900">
      ...
      <div className="bg-hyperion-card border border-slate-800/50 rounded-3xl overflow-hidden shadow-2xl">
        <div className="p-5 border-b border-slate-800/50 bg-slate-900/10 flex justify-between items-center">
          <h3 className="text-sm font-semibold text-slate-300 uppercase tracking-wider flex items-center gap-2">
            <FileText size={16} className="text-blue-400" /> Matriz de Políticas del SGSI
          </h3>
        </div>
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-800/40">
            {politicas.map((pol) => (
              <tr key={pol.id} className="hover:bg-blue-500/[0.01] transition-colors">
                <td className="px-6 py-4">
                  <div className="flex flex-col">
                    <span className="text-slate-200 font-medium text-xs md:text-sm">{pol.titulo}</span>
                    <span className="text-[10px] font-mono text-slate-500">{pol.id}</span>
                  </div>
                </td>
                ...
                <td className="px-6 py-4 text-right">
                  <button className="text-blue-500 hover:text-blue-400 text-xs font-medium flex items-center gap-1 ml-auto group transition-colors">
