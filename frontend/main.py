...
st.markdown("""
    <style>
        ...
        /* Contenedor del Perfil de Operador (Estructura de la imagen) */
        .user-profile-card {
            display: flex;
            align-items: center;
            gap: 14px;
        ...
        .user-avatar {
            width: 42px;
            height: 42px;
            background-color: #00b074; /* Tono verde esmeralda de la imagen */
            border-radius: 12px;
            display: flex;
            align-items: center;
        ...
        /* Buscador Simulado Premium (Estructura de la imagen) */
...
# --- FLUJO 2: INTERFAZ DE VISTAS PROTEGIDAS ---
    ...
    # BARRA LATERAL FIJA PERSISTENTE (Estructura Estética Clonada)
    with st.sidebar:
        ...
        # Estado del Sistema de forma compacta
        try:
        ...
        # 1. Bloque de Perfil del Operador (Estructura de la imagen)
        operator_initial = st.session_state.auth["user"][0].upper() if st.session_state.auth["user"] else "O"
        short_name = st.session_state.auth["user"].split("@")[0].upper() if st.session_state.auth["user"] else "OPERATOR"
        st.markdown(f"""
            <div class="user-profile-card">
                <div class="user-avatar">{operator_initial}</div>
                <div class="user-info">
                    <span class="user-name">{short_name}</span>
                    <span class="user-role">SOC Active Operator</span>
                </div>
            </div>
        """, unsafe_allow_html=True)
        
        # 2. Buscador Falso/Simulado (Estructura de la imagen)
        st.markdown("""
        ...
        # 3. Lista de Botones de Navegación del Sistema Originales
